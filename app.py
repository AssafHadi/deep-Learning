...
# ------------------------------------------------------------
# Unified single-file shell
# ------------------------------------------------------------
# This file intentionally contains the ANN, CNN, and LSTM apps as real
# Python code inside runner functions. It does NOT store app code in
# string variables and does NOT use exec/compile to launch the models.

_ORIGINAL_SET_PAGE_CONFIG = st.set_page_config


def _patched_set_page_config(*args, **kwargs):
    # The unified shell already owns Streamlit page config.
    # Inner apps may still call st.set_page_config; this prevents duplicate
    # page-config crashes while keeping the original app code intact.
    return None


def _ensure_streamlit_shell() -> str:
    _ORIGINAL_SET_PAGE_CONFIG(
        page_title="Oil & Gas Neural Studio",
        layout="wide",
    ...
    chips = [
        ...
        "<span class='shell-chip'>Real Code: Yes</span>",
    ]
...
def run_cnn_app() -> None:
    ...
    # Plotting
    # -----------------------------
    def _cnn_style_axis(ax, title=None, xlabel=None, ylabel=None, grid_axis="both"):
        if title:
    ...
    def plot_class_distribution(df: pd.DataFrame):
        ...
        labels = [_short_label(x, 24) for x in counts.index]
        ax.barh(labels, counts.values)
        _cnn_style_axis(ax, "Class Distribution", "Images", "Class", grid_axis="x")
    ...
    def plot_image_dimensions(df: pd.DataFrame):
        ...
        with col1:
            ...
            _cnn_style_axis(ax, "Image Width vs Height", "Width (px)", "Height (px)")
            fig.tight_layout()
            fig_show(fig)
        ...
        with col2:
            ...
            _cnn_style_axis(ax, "Aspect Ratio Distribution", "Width / Height", "Images", grid_axis="y")
            fig.tight_layout()
            fig_show(fig)
    ...
    def plot_training_curves(history: dict):
        ...
        with col1:
            ...
            _cnn_style_axis(ax, "Training Loss vs Validation Loss", "Epoch", "Loss")
        ...
        with col2:
            ...
            if not plotted:
                ...
                for c in numeric_cols[:2]:
                    ...
                    plotted = True
            _cnn_style_axis(ax, "Training Accuracy vs Validation Accuracy", "Epoch", "Accuracy")
            if plotted:
        ...
        if "lr" in df:
            ...
            _cnn_style_axis(ax, "Learning Rate Schedule", "Epoch", "Learning Rate")
            fig.tight_layout()
            fig_show(fig)
    ...
    def plot_confusion_matrices(cm, class_names):
        ...
        labels = [_short_label(c, 14) for c in class_names]
        ...
        with col1:
            ...
            _cnn_style_axis(ax, "Confusion Matrix", "Predicted", "Actual", grid_axis="none")
        ...
        with col2:
            ...
            _cnn_style_axis(ax, "Normalized Confusion Matrix", "Predicted", "Actual", grid_axis="none")
            ax.set_xticks(np.arange(len(labels)))
            ax.set_xticklabels(labels, rotation=35, ha="right")
            ax.set_yticks(np.arange(len(labels)))
            ax.set_yticklabels(labels)
    ...
    def plot_per_class_metrics(report_dict, class_names):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([_short_label(c, 15) for c in df["class"]], rotation=35, ha="right")
        ax.set_ylim(0, 1.05)
        _cnn_style_axis(ax, "Per-Class Precision / Recall / F1", "Class", "Score", grid_axis="y")
            ...
            if "binary" in roc_info:
                ...
                for cname in class_names[:8]:
                    if cname in roc_info:
                        d = roc_info[cname]
                        ax.plot(d["fpr"], d["tpr"], linewidth=1.35, label=f"{_short_label(cname, 10)} ({d['auc']:.2f})")
                        plotted = True
            ax.plot([0, 1], [0, 1], linestyle="--", alpha=0.55, linewidth=1.0)
            _cnn_style_axis(ax, "ROC Curve", "False Positive Rate", "True Positive Rate")
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1.02)
            if plotted:
        ...
        with col2:
            ...
            if "binary" in pr_info:
                ...
                for cname in class_names[:8]:
                    if cname in pr_info:
                        d = pr_info[cname]
                        ax.plot(d["recall"], d["precision"], linewidth=1.35, label=f"{_short_label(cname, 10)} ({d['ap']:.2f})")
                        plotted = True
            _cnn_style_axis(ax, "Precision-Recall Curve", "Recall", "Precision")
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1.02)
            if plotted:
    ...
    def plot_prediction_confidence_distribution(eval_artifacts):
        ...
        with col1:
            ...
            _cnn_style_axis(ax, "Prediction Confidence Distribution", "Max Class Probability", "Images", grid_axis="y")
            ax.set_xlim(0, 1.0)
        ...
        with col2:
            ...
            if rows:
                ...
                fig, ax = plt.subplots(figsize=(4.35, 2.55))
                ax.barh([_short_label(c, 18) for c in plot_df["class"]], plot_df["mean_confidence"])
                _cnn_style_axis(ax, "Mean Confidence by Predicted Class", "Mean Confidence", "Predicted Class", grid_axis="x")
                ax.set_xlim(0, 1.0)
    ...
    def plot_correct_incorrect_summary(eval_artifacts):
        y_true = np.asarray(eval_artifacts.get("y_true", []))
        y_pred = np.asarray(eval_artifacts.get("y_pred", []))
        class_names = eval_artifacts.get("class_names", [])
        ...
        with col1:
            ...
            _cnn_style_axis(ax, "Correct vs Incorrect Predictions", "Prediction Result", "Images", grid_axis="y")
        ...
        with col2:
            ...
            if rows:
                ...
                fig, ax = plt.subplots(figsize=(4.1, 2.45))
                ax.barh([_short_label(c, 18) for c in plot_df["class"]], plot_df["accuracy"])
                _cnn_style_axis(ax, "Per-Class Validation Accuracy", "Accuracy", "Class", grid_axis="x")
                ax.set_xlim(0, 1.0)
    ...
    def show_correct_incorrect_examples(eval_artifacts, max_each=4):
        y_true = np.asarray(eval_artifacts["y_true"])
        y_pred = np.asarray(eval_artifacts["y_pred"])
        probs = np.asarray(eval_artifacts["y_prob"])
        paths = eval_artifacts["val_paths"]
    ...
    def plot_embedding_map(model, eval_artifacts, cfg):
        ...
        for i, cname in enumerate(class_names):
            ...
            ax.scatter(emb[mask, 0], emb[mask, 1], label=_short_label(cname, 12), alpha=0.74, s=18)
        _cnn_style_axis(ax, "Validation Feature Embedding (PCA)", "PC 1", "PC 2")
    ...
    def page_evaluate():
        ...
        plot_prediction_confidence_distribution(ev)
        plot_correct_incorrect_summary(ev)
        ...
        show_correct_incorrect_examples(ev, max_each=4)
...
def run_lstm_app() -> None:
    ...
    def preprocess_dataset(df: pd.DataFrame, cfg: Dict) -> Dict:
        ...
        feature_df = coerce_numeric(work[feature_cols], feature_cols)
        ...
        feature_df = apply_missing(feature_df, cfg["missing_method"])
        if cfg["clip_outliers"]:
            feature_df = clip_quantiles(feature_df, cfg["clip_low_q"], cfg["clip_high_q"])

        transformed_features = apply_transform(feature_df, cfg["transform_mode"])
        transformed_features = apply_missing(transformed_features, cfg["missing_method"])
        ...
        seq = create_classification_sequences(
            X_scaled,
            y_labels,
            dates.to_numpy(),
            lookback,
    ...
    def build_model(input_shape: Tuple[int, int], num_targets: int, horizon: int, cfg: Dict, n_classes: Optional[int] = None):
        tf = get_tf()
    ...
    def train_model(processed: Dict, cfg: Dict) -> Dict:
        ...
        model = build_model(processed["X_train"].shape[1:], num_targets, horizon, cfg, n_classes=n_classes)
        ...
        if task == "classification":
            ...
            metrics_tables = {
                split: compute_classification_metrics_table(actuals[split], predictions[split], probabilities[split], processed["class_names"])
    ...
    def compute_classification_metrics_table(y_true: np.ndarray, y_pred: np.ndarray, y_prob: np.ndarray, class_names: List[str]) -> pd.DataFrame:
    ...
    # Plot helpers
    # -----------------------------
    # Small, report-ready LSTM plots. Keep these compact; Streamlit columns will
    # otherwise make the old graphs look oversized and messy.
    PLOT_W = 5.15
    PLOT_H = 2.55
    PLOT_WIDE = 6.4
    PLOT_DPI = 130
    ...
    def _new_fig(width: float = PLOT_W, height: float = PLOT_H):
        fig, ax = plt.subplots(figsize=(width, height), dpi=PLOT_DPI)
        fig.patch.set_facecolor("white")
        ax.set_facecolor("white")
        return fig, ax


    def _style_ax(ax, title: str, xlabel: Optional[str] = None, ylabel: Optional[str] = None, legend: bool = True):
        ax.set_title(title, fontsize=10.5, fontweight="bold", color=LSTM_COLORS["text"], pad=8)
        if xlabel:
            ax.set_xlabel(xlabel, fontsize=8.6, color=LSTM_COLORS["text"])
        if ylabel:
            ax.set_ylabel(ylabel, fontsize=8.6, color=LSTM_COLORS["text"])
        ax.grid(True, alpha=0.34, linewidth=0.65, color=LSTM_COLORS["grid"])
    ...
    def _valid_pair(actual, pred) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _downsample_pair(a: np.ndarray, p: np.ndarray, max_points: int = MAX_PLOT_POINTS) -> Tuple[np.ndarray, np.ndarray]:
        if len(a) <= max_points:
            return a, p
    ...
    def _clean_timeseries_frame(df: pd.DataFrame) -> pd.DataFrame:
        """
        Plot-only cleanup.
        ...
        return _downsample_df(out)


    def _target_label_from_text(text: str = "", transform_mode: str = "raw") -> str:
    ...
    def _target_label_from_columns(cols: List[str], transform_mode: str = "raw") -> str:
        joined = " ".join([str(c) for c in cols])
        return _target_label_from_text(joined, transform_mode=transform_mode)


    def _format_date_axis(fig, ax):
        try:
    ...
    def fig_line(df: pd.DataFrame, title: str, ylabel: str = "Value"):
        clean = _clean_timeseries_frame(df)
        fig, ax = _new_fig(PLOT_WIDE, PLOT_H)
        if clean.empty:
            ax.text(0.5, 0.5, "No numeric data to plot", ha="center", va="center", transform=ax.transAxes)
        else:
            for i, col in enumerate(clean.columns):
                color_cycle = [LSTM_COLORS["blue"], LSTM_COLORS["orange"], LSTM_COLORS["green"], LSTM_COLORS["purple"], LSTM_COLORS["red"]]
                ax.plot(
                    clean.index,
                    clean[col],
                    label=str(col),
        ...
        _format_date_axis(fig, ax)
        _style_ax(ax, title, xlabel="Time", ylabel=ylabel, legend=True)
        return _finish_fig(fig)


    def fig_training_history(history: Dict):
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        loss = np.asarray(history.get("loss", []), dtype=float)
        val_loss = np.asarray(history.get("val_loss", []), dtype=float)

        if len(loss):
            ax.plot(loss, label="Train Loss", linewidth=1.8, color=LSTM_COLORS["blue"])
        if len(val_loss):
            ax.plot(val_loss, label="Validation Loss", linewidth=1.6, color=LSTM_COLORS["orange"], linestyle="--")
            best_epoch = int(np.nanargmin(val_loss))
            ax.axvline(best_epoch, linestyle=":", linewidth=1.0, color=LSTM_COLORS["gray"], label=f"Best epoch {best_epoch + 1}")
        ...
        _style_ax(ax, "Training Loss vs Validation Loss", xlabel="Epoch", ylabel="Loss", legend=True)
        return _finish_fig(fig)


    def fig_actual_vs_predicted(actual: np.ndarray, pred: np.ndarray, title: str):
        a, p = _valid_pair(actual, pred)
        a_plot, p_plot = _downsample_pair(a, p)
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        if len(a_plot):
            ax.plot(a_plot, label="Actual", linewidth=1.65, color=LSTM_COLORS["blue"])
            ax.plot(p_plot, label="LSTM Predicted", linewidth=1.55, color=LSTM_COLORS["orange"], linestyle="--")
        _style_ax(ax, title, xlabel="Sample", ylabel=_target_label_from_text(title), legend=True)
        return _finish_fig(fig)
    ...
    def fig_distribution(actual: np.ndarray, pred: np.ndarray, title: str):
        ...
        if len(a):
            ...
            ax.hist(a, bins=bins, alpha=0.48, label="Actual", color=LSTM_COLORS["blue"], edgecolor="white", linewidth=0.35)
            ax.hist(p, bins=bins, alpha=0.48, label="Predicted", color=LSTM_COLORS["orange"], edgecolor="white", linewidth=0.35)
        _style_ax(ax, title, xlabel=_target_label_from_text(title), ylabel="Frequency", legend=True)
        return _finish_fig(fig)


    def fig_scatter(actual: np.ndarray, pred: np.ndarray, title: str):
        a, p = _valid_pair(actual, pred)
        a_plot, p_plot = _downsample_pair(a, p, max_points=900)
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        if len(a_plot):
            ax.scatter(a_plot, p_plot, alpha=0.52, s=13, color=LSTM_COLORS["blue"], linewidths=0)
        ...
        _style_ax(ax, "Actual vs Predicted Scatter", xlabel="Actual", ylabel="Predicted", legend=True)
        return _finish_fig(fig)
    ...
    def fig_residuals_vs_pred(pred: np.ndarray, residuals: np.ndarray, title: str):
        ...
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        if len(p):
            ax.scatter(p, r, alpha=0.50, s=13, color=LSTM_COLORS["blue"], linewidths=0)
            ax.axhline(0, linewidth=1.05, color="#111827", linestyle="--", label="Zero error")
            std = float(np.nanstd(r))
            if np.isfinite(std) and std > 0:
                ax.axhline(2 * std, linestyle=":", linewidth=0.9, color=LSTM_COLORS["gray"], label="±2σ")
                ax.axhline(-2 * std, linestyle=":", linewidth=0.9, color=LSTM_COLORS["gray"])
        _style_ax(ax, title, xlabel="Predicted", ylabel="Residual", legend=True)
        return _finish_fig(fig)


    def fig_residual_hist(residuals: np.ndarray, title: str):
        r = _numeric_array(residuals)
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        if len(r):
            bins = min(34, max(12, int(np.sqrt(len(r)))))
            ax.hist(r, bins=bins, alpha=0.78, color=LSTM_COLORS["blue"], edgecolor="white", linewidth=0.4)
            ax.axvline(0, linestyle="--", linewidth=1.0, color="#111827", label="Zero error")
            ax.axvline(np.nanmean(r), linestyle=":", linewidth=1.0, color=LSTM_COLORS["orange"], label="Mean")
        _style_ax(ax, "Residual Histogram", xlabel="Residual", ylabel="Frequency", legend=True)
        return _finish_fig(fig)
    ...
    def fig_qq(residuals: np.ndarray, title: str):
        ...
        if len(r) >= 3:
            ...
            if len(ax.lines) >= 1:
                ax.lines[0].set_markerfacecolor(LSTM_COLORS["blue"])
                ax.lines[0].set_markeredgecolor(LSTM_COLORS["blue"])
                ax.lines[0].set_markersize(3.0)
                ax.lines[0].set_alpha(0.62)
            if len(ax.lines) >= 2:
                ax.lines[1].set_color(LSTM_COLORS["orange"])
                ax.lines[1].set_linewidth(1.2)
        _style_ax(ax, title, xlabel="Theoretical Quantiles", ylabel="Ordered Residuals", legend=False)
        return _finish_fig(fig)
    ...
    def fig_rolling_rmse(actual: np.ndarray, pred: np.ndarray, title: str, window: int = 30):
        ...
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        ax.plot(rolling.values, linewidth=1.6, color=LSTM_COLORS["blue"])
        _style_ax(ax, title, xlabel="Sample", ylabel="Rolling RMSE", legend=False)
        return _finish_fig(fig)
    ...
    def fig_correlation_heatmap(df: pd.DataFrame, title: str):
        ...
        if not corr.empty:
            ...
            fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        _style_ax(ax, title, legend=False)
        return _finish_fig(fig)
    ...
    def fig_missing_bars(df: pd.DataFrame, title: str):
        ...
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        ax.bar(miss.index.astype(str), miss.values, color=LSTM_COLORS["blue"], alpha=0.82)
        ax.tick_params(axis="x", rotation=35)
        _style_ax(ax, title, ylabel="Missing Count", legend=False)
        return _finish_fig(fig)
    ...
    def fig_split_overview(dates: pd.Series, train_end_idx: int, val_end_idx: int, title: str):
        ...
        fig, ax = _new_fig(PLOT_WIDE, 1.55)
        if len(dates):
            train_end_idx = min(max(int(train_end_idx), 1), len(dates) - 1)
            val_end_idx = min(max(int(val_end_idx), train_end_idx + 1), len(dates) - 1)
            ax.axvspan(dates.iloc[0], dates.iloc[train_end_idx - 1], alpha=0.18, color=LSTM_COLORS["blue"], label="Train")
            ax.axvspan(dates.iloc[train_end_idx], dates.iloc[val_end_idx - 1], alpha=0.18, color=LSTM_COLORS["orange"], label="Validation")
            ax.axvspan(dates.iloc[val_end_idx], dates.iloc[-1], alpha=0.18, color=LSTM_COLORS["green"], label="Test")
            ax.plot(dates, np.ones(len(dates)), alpha=0)
        ax.set_yticks([])
        _format_date_axis(fig, ax)
        _style_ax(ax, title, xlabel="Time", legend=True)
        return _finish_fig(fig)


    def fig_class_distribution(y_true: np.ndarray, y_pred: np.ndarray, class_names: List[str], title: str):
        y_true = np.asarray(y_true).astype(int).reshape(-1)
        y_pred = np.asarray(y_pred).astype(int).reshape(-1)
        ...
        width = 0.38
        ax.bar(x - width / 2, true_counts, width=width, label="Actual", color=LSTM_COLORS["blue"], alpha=0.82)
        ax.bar(x + width / 2, pred_counts, width=width, label="Predicted", color=LSTM_COLORS["orange"], alpha=0.82)
        ax.set_xticks(x)
        ax.set_xticklabels(class_names, rotation=25, ha="right")
        _style_ax(ax, title, xlabel="Class", ylabel="Count", legend=True)
        return _finish_fig(fig)


    def fig_lstm_confusion(y_true: np.ndarray, y_pred: np.ndarray, class_names: List[str], title: str):
        ...
        _style_ax(ax, title, xlabel="Predicted", ylabel="Actual", legend=False)
        return _finish_fig(fig)


    def fig_class_probability_hist(y_prob: np.ndarray, y_pred: np.ndarray, class_names: List[str], title: str):
        ...
        fig, ax = _new_fig(PLOT_W, PLOT_H)
        if len(conf):
            ax.hist(conf, bins=min(30, max(8, int(np.sqrt(len(conf))))), color=LSTM_COLORS["blue"], alpha=0.78, edgecolor="white", linewidth=0.4)
            ax.axvline(np.nanmean(conf), color=LSTM_COLORS["orange"], linestyle="--", linewidth=1.1, label="Mean confidence")
        _style_ax(ax, title, xlabel="Predicted Class Confidence", ylabel="Frequency", legend=True)
        return _finish_fig(fig)


    def fig_binary_roc_pr(y_true: np.ndarray, y_prob: np.ndarray, title_prefix: str):
        y_true = np.asarray(y_true).astype(int).reshape(-1)
        score = np.asarray(y_prob)[:, 1]
        fig1, ax1 = _new_fig(PLOT_W, PLOT_H)
        try:
            fpr, tpr, _ = roc_curve(y_true, score)
            ax1.plot(fpr, tpr, color=LSTM_COLORS["blue"], linewidth=1.7, label="ROC")
            ax1.plot([0, 1], [0, 1], linestyle="--", color=LSTM_COLORS["gray"], linewidth=1.0, label="Random")
            _style_ax(ax1, f"{title_prefix} ROC Curve", xlabel="False Positive Rate", ylabel="True Positive Rate", legend=True)
        except Exception:
            _style_ax(ax1, f"{title_prefix} ROC Curve", xlabel="False Positive Rate", ylabel="True Positive Rate", legend=False)

        fig2, ax2 = _new_fig(PLOT_W, PLOT_H)
        try:
            precision, recall, _ = precision_recall_curve(y_true, score)
            ax2.plot(recall, precision, color=LSTM_COLORS["orange"], linewidth=1.7, label="PR")
            _style_ax(ax2, f"{title_prefix} Precision-Recall Curve", xlabel="Recall", ylabel="Precision", legend=True)
        except Exception:
            _style_ax(ax2, f"{title_prefix} Precision-Recall Curve", xlabel="Recall", ylabel="Precision", legend=False)
        return _finish_fig(fig1), _finish_fig(fig2)


    def fig_equity_curves(equity_df: pd.DataFrame, title: str):
        clean = _downsample_df(equity_df.copy())
        fig, ax = _new_fig(PLOT_WIDE, PLOT_H)
        for i, col in enumerate(clean.columns):
            color_cycle = [LSTM_COLORS["blue"], LSTM_COLORS["orange"], LSTM_COLORS["green"], LSTM_COLORS["purple"]]
            ax.plot(clean.index, clean[col], label=col, linewidth=1.55, color=color_cycle[i % len(color_cycle)])
        _format_date_axis(fig, ax)
        _style_ax(ax, title, xlabel="Time", ylabel="Equity (Start = 1.0)", legend=True)
        return _finish_fig(fig)


    def fig_drawdowns(equity_df: pd.DataFrame, title: str):
        dd = equity_df / equity_df.cummax() - 1.0
        dd = _downsample_df(dd)
        fig, ax = _new_fig(PLOT_WIDE, PLOT_H)
        for i, col in enumerate(dd.columns):
            color_cycle = [LSTM_COLORS["blue"], LSTM_COLORS["orange"], LSTM_COLORS["green"], LSTM_COLORS["purple"]]
            ax.plot(dd.index, dd[col], label=col, linewidth=1.45, color=color_cycle[i % len(color_cycle)])
        _format_date_axis(fig, ax)
        _style_ax(ax, title, xlabel="Time", ylabel="Drawdown", legend=True)
        return _finish_fig(fig)
    ...
    def page_model():
        hero("Model", "Choose the LSTM task and architecture. Data columns and splits belong in Preprocess.")
        df = st.session_state.get("raw_df")
        if df is None:
    ...
    def page_preprocess():
        hero("Preprocess", "Choose columns, clean the series, split it sequentially, and build LSTM windows.")
        df = st.session_state.get("raw_df")
        if df is None:
            st.info("Load data first.")
            return
        ...
        if processed is not None:
            ...
            st.pyplot(fig_split_overview(processed["dates"], processed["train_end_idx"], processed["val_end_idx"], "Sequential Train / Validation / Test Split"), use_container_width=True)
            st.dataframe(
                pd.DataFrame({
    ...
    def page_train():
        hero("Train", "Train the selected LSTM task using the already-preprocessed sequence data.")
        processed = st.session_state.get("processed")
        ...
        if processed.get("task") == "regression":
            cfg["loss"] = st.selectbox("Regression loss", ["mse", "mae", "huber"], index=["mse", "mae", "huber"].index(cfg["loss"]))
    ...
    def page_evaluate():
        hero("Evaluate", "Evaluate the selected LSTM task with the right metrics. Regression and classification are not mixed.")
        training = st.session_state.get("training")
        processed = st.session_state.get("processed")
        ...
        if processed.get("task") == "classification":
            ...
            a, b = st.columns(2)
            with a:
                show_lstm_fig(fig_lstm_confusion(y_true, y_pred, class_names, f"{split.title()} Confusion Matrix"))
            with b:
                show_lstm_fig(fig_class_distribution(y_true, y_pred, class_names, f"{split.title()} Actual vs Predicted Class Distribution"))

            c, d = st.columns(2)
            with c:
                show_lstm_fig(fig_class_probability_hist(y_prob, y_pred, class_names, f"{split.title()} Prediction Confidence"))
            with d:
            ...
            if len(class_names) == 2:
                fig1, fig2 = fig_binary_roc_pr(y_true, y_prob, split.title())
                e, f = st.columns(2)
                with e:
                    show_lstm_fig(fig1)
                with f:
        ...
        a, b = st.columns(2)
        with a:
            show_lstm_fig(fig_actual_vs_predicted(actual, pred, f"{split.title()} | {target} | Step {horizon_step}: Actual vs Predicted"))
        with b:
            show_lstm_fig(fig_scatter(actual, pred, f"{split.title()} | {target} | Scatter"))

        c, d = st.columns(2)
        with c:
            show_lstm_fig(fig_residuals_vs_pred(pred, residuals, f"{split.title()} | {target} | Residuals vs Predicted"))
        with d:
            show_lstm_fig(fig_residual_hist(residuals, f"{split.title()} | {target} | Residual Histogram"))

        e, f = st.columns(2)
        with e:
            show_lstm_fig(fig_distribution(actual, pred, f"{split.title()} | {target} | Actual vs Predicted Distribution"))
        with f:
            show_lstm_fig(fig_qq(residuals, f"{split.title()} | {target} | Residual QQ Plot"))
    ...
    def prepare_prediction_input(df_new: pd.DataFrame, processed: Dict, cfg: Dict) -> Tuple[np.ndarray, pd.DataFrame]:
        ...
        numeric = coerce_numeric(work[feature_cols], feature_cols)
        numeric = apply_missing(numeric, cfg["missing_method"])

        if processed.get("task") == "regression":
            numeric = apply_transform(numeric, cfg["transform_mode"])
            numeric = apply_missing(numeric, cfg["missing_method"])
        else:
            numeric = apply_transform(numeric, cfg["transform_mode"])
            numeric = apply_missing(numeric, cfg["missing_method"])
    ...
    def page_predict():
        hero("Predict", "Generate numeric forecasts or class predictions from the latest sequence window.")
        training = st.session_state.get("training")
        processed = st.session_state.get("processed")
        if training is None or processed is None:
            st.info("Train the model first.")
        ...
        if st.button(button_label, type="primary", use_container_width=True):
            try:
                ...
                if processed.get("task") == "classification":
                    ...
                    pred_inv = inverse_3d(pred_raw, processed["target_scaler"], int(cfg["horizon"]), len(processed["target_cols"]))[0]
    ...
    def page_visualize():
        hero("Visualize", "Task-aware plots: regression diagnostics for forecasting, classification diagnostics for class prediction.")
        training = st.session_state.get("training")
        processed = st.session_state.get("processed")
        if training is None or processed is None:
            st.info("Train the model first.")
        ...
        if isinstance(processed["dates"].iloc[0], pd.Timestamp):
            ...
            with ov1:
                if processed.get("task") == "regression":
                    y_label = _target_label_from_columns(processed["target_cols"], cfg.get("transform_mode", "raw"))
                    show_lstm_fig(fig_line(frame[processed["target_cols"]], "Target Series Over Time", ylabel=y_label))
                else:
                    show_lstm_fig(fig_line(frame[processed["feature_cols"][: min(4, len(processed["feature_cols"]))]], "Input Feature Series Over Time", ylabel="Transformed / Scaled Feature Value"))
        else:
            with ov1:
                st.info("Date-based overview is only shown when a valid date column exists.")
        with ov2:
            show_lstm_fig(fig_training_history(training["history"]))
        ...
        if processed.get("task") == "classification":
            ...
            with a:
                show_lstm_fig(fig_lstm_confusion(y_true, y_pred, class_names, f"{split.title()} Confusion Matrix"))
            with b:
                show_lstm_fig(fig_class_distribution(y_true, y_pred, class_names, f"{split.title()} Class Distribution"))
        ...
        a, b = st.columns(2)
        with a:
            show_lstm_fig(fig_actual_vs_predicted(actual, pred, f"{split.title()} | {target} | Actual vs Predicted"))
        with b:
            show_lstm_fig(fig_scatter(actual, pred, f"{split.title()} | {target} | Scatter"))
        ...
        with c:
            show_lstm_fig(fig_residuals_vs_pred(pred, residuals, f"{split.title()} | {target} | Residuals vs Predicted"))
        with d:
            show_lstm_fig(fig_rolling_rmse(actual, pred, f"{split.title()} | {target} | Rolling RMSE", window=max(10, int(cfg['lookback']))))

        e, f = st.columns(2)
        with e:
            show_lstm_fig(fig_distribution(actual, pred, f"{split.title()} | {target} | Distribution"))
        with f:
            show_lstm_fig(fig_qq(residuals, f"{split.title()} | {target} | QQ Plot"))
        ...
        if cfg["transform_mode"] in {"pct_change", "log_return"} and int(cfg["horizon"]) >= 1:
            ...
            bt_returns = returns_for_backtest(actual_first, pred_first)
            equity, dd = backtest_strategies(bt_returns[0], bt_returns[1], dates, processed["target_cols"], processed["freq_name"])
            g, hcol = st.columns(2)
            with g:
                show_lstm_fig(fig_equity_curves(equity, "Strategy Equity Curves"))
            with hcol:
                show_lstm_fig(fig_drawdowns(dd, "Strategy Drawdowns"))
...
def main() -> None:
    ...
    # Prevent duplicate page-config crashes from the selected inner app.
